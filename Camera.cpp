...
Camera::Camera(Mat4& mat, double aspect_ratio, double vertical_field_of_view)
{
	this->mat = mat;
	set_perspective(aspect_ratio, vertical_field_of_view);
}
...
double near_clipping_plane = 0.001;
#define FAR		(TAU)

void Camera::set_perspective(double ar, double vfov)
{
	aspect_ratio = ar;
	double q = tan(0.5 * vfov);

	#define near (near_clipping_plane)
	projection = Mat4(
		1.0 / (ar * q),		0,				0,								0,
		0,					-1.0 / q,		0,								0,
